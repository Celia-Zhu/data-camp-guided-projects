...
```{r Load packages and dataset, warning=FALSE, message=FALSE}
# Load relevant packages
package_list <- c("tidyverse", "dplyr", "readr", "ggplot2", "cluster", "factoextra")
lapply(package_list, require, character.only = TRUE)

# Read in the dataset
degrees <- read_csv("datasets/degrees-that-pay-back.csv", 
                    col_names=c("College.Major", "Starting.Median.Salary", "Mid.Career.Median.Salary", "Career.Percent.Growth", "Percentile.10", "Percentile.25", "Percentile.75", "Percentile.90"), 
                    skip=1)
...
```
## Data cleaning: currency, strings and percents
...
degrees_clean <- degrees %>% 
    ...
    mutate(Career.Percent.Growth = Career.Percent.Growth/100) # Strip dollar sign
...
```{r Elbow method, warning=FALSE, message=FALSE}
...
```{r Silhouette method, warning=FALSE, message=FALSE}
...
```{r Gap statistic method, warning=FALSE, message=FALSE}
...
```{r K-means algorithm, warning=FALSE, message=FALSE}
# Set a random seed
set.seed(111) # To make sure every randomization yield to the same result

# Set k equal to the optimal number of clusters
num_clusters <- 3

# Run the k-means algorithm 
k_means <- kmeans(k_means_data, centers = num_clusters, iter.max = 15,
                 nstart = 25)
...
```{r, Vis clusters, warning=FALSE, message=FALSE}
...
career_growth <- ggplot(degrees_labeled, aes(x = Starting.Median.Salary,
                ...
                labs(x = "Starting Median Salary",
                    y = "Mid Career Median Salary",
                    title = "Relationship between Starting and Mid Career Salaries by Cluster")+
                scale_x_continuous(labels = scales::dollar)+
                scale_y_continuous(labels = scales::dollar)+
                scale_color_manual(values = c("#EC2C73", "#29AEC7", "#FFDD30"))

# View the plot
career_growth
```

## A deeper dive into the cluters
...
```{r Reshape data, warning=FALSE, message=FALSE}
...
Let's graph Cluster 3 and examine the results. These Liberal Arts majors may represent the lowest percentiles with limited growth opportunity, but there is hope for those who make it! Music is our riskiest major with the lowest 10th percentile salary, but Drama wins the highest growth potential in the 90th percentile for this cluster (so don't let go of those Hollywood dreams!). Nursing is the outlier culprit of cluster number 3, with a higher safety net in the lowest percentile to the median. Otherwise, this cluster does represent the majors with limited growth opportunity.

An aside: It's worth noting that most of these majors leading to lower-paying jobs are women-dominated, according to this [Glassdoor study](https://www.glassdoor.com/research/app/uploads/sites/2/2017/04/FULL-STUDY-PDF-Gender-Pay-Gap2FCollege-Major.pdf). According to the research: 
...
```{r Plot cluster 3, warning=FALSE, message=FALSE}
# Graph the majors of Cluster 1 by percentile
cluster_3 <- degrees_perc  %>% 
            filter(clusters == 3) %>% 
            ggplot(aes(x = percentile, y = salary, 
                       color = College.Major, group = College.Major))+
            geom_point(alpha = 0.8)+
            geom_line(alpha = 0.5)+
            labs(title = "Cluster 3: The Liberal Arts")+
...
On to Cluster 1, right in the middle! Accountants are known for having stable job security, but once you're in the big leagues you may be surprised to find that Marketing or Philosophy can ultimately result in higher salaries. The majors of this cluster are fairly middle of the road in our dataset, starting off not too low and not too high in the lowest percentile. However, this cluster also represents the majors with the greatest differential between the lowest and highest percentiles.
```{r Plot cluster 1, warning=FALSE, message=FALSE}
# Modify the previous plot to display Cluster 1
# Graph the majors of Cluster 1 by percentile
cluster_1 <- degrees_perc  %>% 
            ...
            geom_point(alpha = 0.8)+
            geom_line(alpha = 0.5)+
            labs(title = "Cluster 1: The Goldilocks")+
            theme(axis.text.x = element_text(angle = 25, size = 7))
...
cluster_2 <- degrees_perc  %>% 
            ...
            geom_point(alpha = 0.8)+
            geom_line(alpha = 0.5)+
            labs(title = "Cluster 2: The Over Achievers")+
            theme(axis.text.x = element_text(angle = 25, size = 7))
...
```{r Find two careers tied for the highest percent growth, warning=FALSE, message=FALSE}
# Sort degrees by Career.Percent.Growth
degrees_labeled <- degrees_labeled %>% 
                   arrange(desc(Career.Percent.Growth))

# Identify the two majors tied for highest career growth potential
highest_career_growth <- top_n(degrees_labeled, 2, Career.Percent.Growth)
